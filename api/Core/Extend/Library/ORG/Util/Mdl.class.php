...
class Mdl
{
    ...
    ///mdl
    const ACKEY_AUTH_PREFIX = 'http://woaapapi.etocrm.com';
    public $ackey;
    private $appkey;
    ...
    public function getOauthRedirect($callback,$state='',$scope='snsapi_userinfo'){
        return self::ACKEY_AUTH_PREFIX.'/api/oauth2-authorize?appid='.$this->appid.'&redirect_uri='.urlencode($callback).'&scope='.$scope.'';
    }

    /**
     * 取得ACKEY
     */
    private function getAckey($appid='',$appkey=''){
        if (!$appid || !$appkey) {
            $appid = $this->appid;
            $appkey = $this->appkey;
        }

        $authname = 'wechat_ackey'.$appid;
        ...
        $result = $this->http_get(self::ACKEY_AUTH_PREFIX.'/api/ackey?appid='.$appid.'&appkey='.$appkey);
        ...
        if ($result)
        {
            ...
            $this->setCache($authname,$this->ackey,$expire);
            return $this->ackey;
        }
        return false;
    }
    ...
    public function getAccessToken(){
        ...
        $result = $this->http_get(self::ACKEY_AUTH_PREFIX.'/api/oauth2-accesstoken?ackey='.$this->ackey.'&code='.$code.'');
    ...
    public function getUserInfo($openid){
        ...
        if (!$this->ackey) return false;

        $result = $this->http_get('http://woaapapi.etocrm.com/api/userinfo?ackey='.$this->ackey.'&openid='.$openid."&lang=zh_CN");
        dump('http://woaapapi.etocrm.com/api/userinfo?ackey='.$this->ackey.'&openid='.$openid."&lang=zh_CN");
        //http://woaaphub.etocrm.com/api/userinfo?ackey=eyJpdiI6IkZycnJnXC9qQWRLZmZRcjBhcnJ1a3B3PT0iLCJ2YWx1ZSI6InlKWUx3WDhoa3JhNXhkSUV2eUlqdDhsTERTR3BFWkNSZ2U2SmxmTlwvTWZCSVhjWDdydE5YSGpuTTFpQU5nRTN4IiwibWFjIjoiMjlkNjgwOGJiZTBkNDQ0YTk4OWUwY2E0ZTg0Zjk0OThkY2Y1MDk3NTJjNTY0NGM2NzJkMDJkNGIwMTY5NDBlYSJ9&openid=oVI6xjrSf_Z4knx3KiBzwGkPNoMY&lang=zh_CN
        //http://woaaphub.etocrm.com/api/userinfo?ackey=ackey&openid=OPENID&lang=zh_CN
        dump($result);
        if ($result)
        {
            $json = json_decode($result,true);
    ...
    public function getJsTicket($appid='',$jsapi_ticket=''){
        ...
        $authname = 'mdl_jsapi_ticket'.$appid;
        if ($rs = $this->getCache($authname))  {
            $this->jsapi_ticket = $rs;
            return $rs;
        }
        $result = $this->http_get(self::ACKEY_AUTH_PREFIX.'/api/jsticket?ackey='.$this->getAckey());
        dump($result);
        if ($result)
        {
            $json = json_decode($result,true);
...
<Nonce><![CDATA[%s]]></Nonce>
     ...
     * @param string $token 手动指定access_token，非必要情况不建议用
     */
    public function checkAckey($appid='',$appkey='',$token=''){
        if (!$appid || !$appkey) {
            $appid = $this->appid;
            $appkey = $this->appkey;
        }
