...
if(gameName == 'elloello'){
    $('.result').addClass('elloello') ;
}else if(gameName == 'babylontower'){
    $('.logo').addClass('babylontower') ;
}
...
function getRecommend() {
    ...
    $.ajax({
        url:'https://platform.shandw.com/ellorecommend',
        type: "GET",
        jsonp: "cb",
        ...
        success:function(res){
            if(res.result === 1){
                ...
                $('#banner .longBanner img').attr('src',res.banner.icon).bind('click',function () {
                    if(res.banner.link){
                        toGame(res.banner.link+'?channel='+urlToObject(location.href)['channel']);
                    }else{
                        toGame(SDW_PATH.MOBILE_ROOT + 'game/'+res.banner.id+'.html?&channel='+urlToObject(location.href)['channel'])
                    }

                   //toGame(SDW_PATH.activities+'nationalFare/?channel='+urlToObject(location.href)['channel'])
                })  ;

                for(var i=0 ;i<$('#banner .command-list img').length;i++){
                    $('#banner .command-list img').eq(i)[0].gid = res.list[i].id ;
                    $('#banner .command-list img').eq(i).attr('src',res.list[i].icon ).bind('click',function () {

                        toGame(SDW_PATH.MOBILE_ROOT + 'game/'+this.gid+'.html?&channel='+urlToObject(location.href)['channel'])
                    });
                    $('#banner .command-list .name').eq(i).html(res.list[i].sub) ;
                }
                function toGame(link) {
...
function initHandle() {
    ...
    $('#banner').hide() ;
    $('.logo').show() ;
    $('#menu').show().removeClass('end').addClass('start') ;
...
function startHandle() {
    ...
    $('#score').show();
    $('#gamebox').addClass('gameboxHide') ;
    $("body").addClass("playing"), $("#score").html("0"),gameStartTimestamp(), "function" == typeof appStartHandle && appStartHandle();
}
...
/*console.log('加载sdk') ;*/
loadJs('https://www.shandw.com/libs/js/sdwJs.min.js?v='+new Date().getTime(),function () {
    loadJs('https://www.shandw.com/libs/path.min.js?v='+new Date().getTime(),function () {
        console.log('sdk加载成功') ;
    })
})
...
function gameover(n) {
    ...
    $('#banner').show() ;
    $('#menu').addClass('end').removeClass('start') ;
    $('#gamebox').removeClass('gameboxHide') ;
    ...
    var roundNum = getQeury('roundNum');
    /*上报游戏数据*/
    upPlayerInfo() ;
    function  upPlayerInfo() {
        var appid = getQeury('appid') , channel = getQeury('channel'),uid = getQeury('uid'),sid = '0' ,level = n.score,type = '休闲',vip = 1,power = 0 ,key  = sdwInfo.key ;
        var sendData = {
            uid : uid,
            gid : gid,
            appid : appid,
    ...
    if(gmUnitId){
        ...
        if(roundNum){
            var part = getQeury('part') , channel = getQeury('channel'),uid = getQeury('uid'),level = n.score,gid=getQeury('gid') ;;
            var url = 'https://platform.shandw.com/telfaregds';
            var sendData={
                roundnum:roundNum,
                part:part,
                uid:uid,
                ...
                level:level,
                gid:gid,
                sign:bgsn(''+roundNum+part+uid+channel+level+gid,true),
            };
...
function resetHandle() {
    ...
    $('#btnXY').hide();
    $('#gamebox').addClass('gameboxHide') ;
    $(".buttons").removeClass("ended"),$("body").addClass("playing"),gameStartTimestamp(3), $("#uistart,#uiend,.logo,.buttons").hide(), "function" == typeof appResetHandle && appResetHandle()
}
